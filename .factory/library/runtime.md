...
- On the baseline mission API path (`3001`), successful malformed-db fallback should settle into the normal ready runtime path when the recovered snapshots are usable. Do not keep `3001` pinned in validation-style `seeded_bootstrap` unless the runtime truly still lacks a usable baseline.
...
- Isolated `3102` startup with usable imported validation snapshots should remain seeded-bootstrap by default; `zero_live_completed_boot` is reserved for the explicit validation override or for a real completed boot with no usable live snapshots, not for every validation-surface boot.
- Port-3000 local bootstrap behavior is legacy repo coverage, not the trust-slice source of truth. If those tests conflict with the approved `3001`/`3102` contract, realign the tests instead of forcing the mission runtime to preserve stale `3000`-only semantics.
